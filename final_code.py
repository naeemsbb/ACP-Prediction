...
from Pfeature.pfeature import aac_wp, atc_wp, sep_wp
import os
import numpy as np
import streamlit as st
...
def main():
    ...
    left_logo.image("university-logo.png", width=280)
    right_logo.image("logopu.png", width=280)
...
def predict_peptide_structure(sequences):
    ...
    sep_df_list = [sep(seq) for seq in sequences if seq]

    df_features = pd.concat([pd.concat(aac_df_list, axis=0), 
                             pd.concat(atc_df_list, axis=0), 
                             pd.concat(sep_df_list, axis=0)], axis=1)

    feature_cols = ['SEP', 'ATC_H', 'ATC_C', 'ATC_O', 'ATC_N', 'ATC_S', 'AAC_E', 'AAC_D', 'AAC_K', 'AAC_C', 'AAC_Q', 'AAC_R', 'AAC_S', 'AAC_T', 'AAC_V', 'AAC_W', 'AAC_L', 'AAC_F', 'AAC_I', 'AAC_G', 'AAC_M', 'AAC_P', 'AAC_Y', 'AAC_H', 'AAC_A', 'AAC_N']
